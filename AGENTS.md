...
`.omp/references/java-domain-architecture-standard.md` 是全仓库 Java 服务端项目的默认领域架构与工程规范，不是技能。它与源码规范并列适用，管理业务领域、职责分层、对象边界、依赖方向、日志、配置、异常、数据访问、事务和跨领域协作。

创建或修改人工编写的 Java 生产或测试源码，或者与其直接配套的应用配置、日志配置、Mapper XML 和基础设施资源文件前，完整读取该规范。先以与任务直接相关的配置、文档和代表性文件判断目标项目是否已有明确且稳定的做法，不为局部任务调查整个仓库。实际规则按以下优先级确定：用户对本次任务的明确要求；项目已生效的显式规范、配置和工具约束；项目或当前模块稳定且占主导的既有实现；默认规范。

只有默认规范与已有项目的明确约定或稳定实现发生冲突时，才按已有项目的做法处理。单个历史文件、偶发写法或已被其他证据否定的实现不构成项目风格。不得借默认规范扩大任务范围或未经要求迁移历史代码；需要改变既有架构时，应作为明确的重构或架构决策处理。

纯文档、纯依赖元数据、纯生成代码，以及与 Java 项目无关的配置或源码任务无需加载该规范。技能可以增加工作流要求，但不能替代或弱化这份默认规范。
