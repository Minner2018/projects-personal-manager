...
该能力解决的是主上下文被大量搜索结果、日志和文件内容占用的问题。它不替代主 AI 的判断，也不改变 OMP skill routing。

## 2. 形态与边界

- 它是内部能力，不是 skill，没有 route token。
- 这里的 OMP 指外部 `omp` CLI 进程，不是 `.omp/skills/` 下的 skill。
- 调用不需要 `[X]` 路由标记；缺少路由标记不能成为不调用该能力的理由。
- 调用前后 active skill 保持不变。
- 主 AI 负责判断是否委派、描述目标、复核证据和形成最终回答。
- OMP 负责在项目中自主定位并阅读相关文本。
- 第一版只允许读取和搜索，不允许写入、执行命令、访问网络、调用 LSP、使用记忆或创建子 Agent。
...
- 当前已激活 `[P]`、`[R]` 等 skill 时，仍可调用 OMP，且 active skill 保持不变。
